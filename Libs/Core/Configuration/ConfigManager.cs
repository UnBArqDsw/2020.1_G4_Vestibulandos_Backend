...
namespace Core.Configuration
{
    ...
    {
        private static Dictionary<string, string> m_dictConfig = new Dictionary<string, string>();

        //---------------------------------------------------------------------------------------------------
        public static bool Load(string fileName, string binaryPath = null)
        {
            string path = (!string.IsNullOrEmpty(binaryPath)) ? Path.Combine(binaryPath, fileName) : AppContext.BaseDirectory + fileName;
            if (!File.Exists(path))
            {
            ...
            try
            {
                foreach (string line in ConfigContent)
                {
                    ...
                    StringArray configOption = new StringArray(line, '=');
                    m_dictConfig.Add(configOption[0].Trim(), configOption[1].Replace("\"", "").Trim());
                }
            }
            catch
            {
        ...
        {
            string temp = m_dictConfig.LookupByKey(name);
        ...
        {
            return m_dictConfig.Where(p => p.Key.Contains(name)).Select(p => p.Key);
        }
    }
}
